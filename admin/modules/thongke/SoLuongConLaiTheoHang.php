...
<body>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            ...
            data.addColumn('number', 'Số lượng');
            // Create Rows with data
            data.addRows([
                <?php
                $conn = connectDB();
                $query = "SELECT count(*) as soluong,hangSX from `product` GROUP BY hangSX";
                $result = mysqli_query($conn, $query);

                if(mysqli_num_rows($result)){
                    while($row = mysqli_fetch_assoc($result)){
                        echo "['".$row['hangSX']."',".$row['soluong']."],";
                    }
                }
                ?>
            ]);
            ...
            var options = {
                title: 'Biểu đồ thông tin sản phẩm còn lại trong kho theo hãng sản xuất',
                'width': 800,
                'height': 600,
            };
